...
type tenhouMessage struct {
	...
	//Machi string `json:"machi"` // 自摸/荣和的牌 126
	// `json:"ten"` // 符数和点数 30,7700,0
	//Yaku       string `json:"yaku"`       // 翻数详情（编号，翻数） 18,1,20,1,34,2
	//DoraHai    string `json:"doraHai"`    // 宝牌 123
	//DoraHaiUra string `json:"doraHaiUra"` // 里宝牌 77
	// `json:"who"` // 和牌者 1
...
var tileReg = regexp.MustCompile(``)
...
func (m *tenhouMessage) analysis(counts []int) error {
	...
	case "INIT":
		...
		if err := _analysis(13, counts); err != nil {
			return err
		}
	default:
		if m.Tag[0] == 'T' {
			...
			counts[index]++
			if err := _analysis(14, counts); err != nil {
				return err
			}
		} else if m.Tag[0] == 'D' {
			index := m._parseTenhouTile(m.Tag[1:])
		...
		} else if tileReg.MatchString(m.Tag) {
			// 舍牌
			//fmt.Println(m.Tag)
		} else {
			//fmt.Println(m.Tag)
